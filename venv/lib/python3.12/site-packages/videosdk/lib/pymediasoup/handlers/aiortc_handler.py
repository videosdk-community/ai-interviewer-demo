...
class AiortcHandler(HandlerInterface):
    ...
    def run(
        ...
        # TODO: will add turn configurations later, since it is not compatible now
    ...
    async def stopSending(self, localId):
        ...
        # offer = await self._pc.createOffer()
        # logger.debug(f'stopSending() | calling pc.setLocalDescription() [offer:{offer}]')
    ...
    async def receive(
        ...
        localId = (
            ...
            if options.rtpParameters.mid != None
        ...
        try:
            await self.pc.setRemoteDescription(
                offer, rtpParameters.codecs, kind, track_buffer_size
            )
        except Exception as e:
            logger.error(f"error while setRemoteDescription {e}")
